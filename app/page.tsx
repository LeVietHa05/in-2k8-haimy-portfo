...
export default function Home() {
  ...
  const [showInstax, setshowinstax] = useState(false)
  const [showDslite, setshowdslite] = useState(false)
  const [showTyper, setshowtyper] = useState(false)
  const [showSwitch, setshowswitch] = useState(false)
  const [showDualsense, setshowdualsense] = useState(false)
  const [showbigbook1, setshowbigbook1] = useState(false)
  const [showbigbook2, setshowbigbook2] = useState(false)
    ...
    <div className="max-w-[1440px] mx-auto relative z-1">
      ...
      <div className="w-160 h-210 mx-auto relative  flex flex-col p-20 pt-32">
        ...
        <div className=" flex z-1 h-40 justify-between items-end mt-12 ml-4">
          ...
          <div className="flex-3" onClick={() => setshowinstax(true)}>
      ...
      {showInstax && createPortal(<Instax onClose={() => setshowinstax(false)} />, document.body)}
      {showDslite && createPortal(<Dslite onClose={() => setshowdslite(false)} />, document.body)}
      {showTyper && createPortal(<Typer onClose={() => setshowtyper(false)} />, document.body)}
      {showSwitch && createPortal(<Switch onClose={() => setshowswitch(false)} />, document.body)}
      {showDualsense && createPortal(<Dualsense onClose={() => setshowdualsense(false)} />, document.body)}
      {showbigbook1 && createPortal(<BookFlip totalPages={30} folder="/bigbook-1" onClose={() => setshowbigbook1(false)} />, document.body)}
      {showbigbook2 && createPortal(<BookFlip totalPages={60} folder="/bigbook-2" onClose={() => setshowbigbook2(false)} />, document.body)}
    </div>
  );
}
